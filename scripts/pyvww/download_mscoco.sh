...
if [ ! -d "${TRAIN_IMAGE_DIR}" ]; then
  ...
  echo "train${YEAR}/ already present, skipping download/unzip"
fi
...
if [ ! -d "${VAL_IMAGE_DIR}" ]; then
  ...
  echo "val${YEAR}/ already present, skipping download/unzip"
fi

COMMON_DIR="all$YEAR"
mkdir -p "${COMMON_DIR}"
for i in "${TRAIN_IMAGE_DIR}"/*; do ln -sf "$i" "${COMMON_DIR}/"; done
for i in "${VAL_IMAGE_DIR}"/*; do ln -sf "$i" "${COMMON_DIR}/"; done
...
if [ ! -d "annotations" ]; then
  ...
  echo "annotations/ already present, skipping download/unzip"
fi
