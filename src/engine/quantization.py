...
def _run_ptq(ctx: Dict, quant_cfg: Dict) -> Dict:
    ...
    qconfig_dict = _build_qconfig_dict(model, get_default_qconfig(backend), exclude_first_last)
    prepared = prepare_pt2e(model, qconfig_dict, example_inputs=example_input)
    ...
    summary = {
        ...
        "state_dict_bytes": state_path.stat().st_size,
        "full_model_bytes": full_path.stat().st_size,
    }
    return summary
...
def _run_qat(ctx: Dict, quant_cfg: Dict) -> Dict:
    ...
    example_input = _example_input(ctx["val_loader"])
    model = ctx["model"]
    qconfig_dict = _build_qconfig_dict(model, get_default_qat_qconfig(backend), exclude_first_last)
    prepared = prepare_qat_pt2e(model, qconfig_dict, example_inputs=example_input)
    prepared.to(device)
...
def _select_backend(name: str) -> str:
    backend = str(name or "auto").lower()
    if backend == "auto":
        arch = platform.machine().lower()
        backend = "qnnpack" if "arm" in arch else "fbgemm"
...
def _build_qconfig_dict(model: nn.Module, default_qconfig, exclude_first_last: bool) -> Dict:
    qconfig_dict: Dict[str, object] = {"": default_qconfig}
    if exclude_first_last:
        first_conv, last_linear = _first_last_module_names(model)
        exclusions = []
        if first_conv:
            exclusions.append((first_conv, None))
        if last_linear:
            exclusions.append((last_linear, None))
        if exclusions:
            qconfig_dict["module_name"] = exclusions
    return qconfig_dict
...
def _save_quantized_model(model: torch.nn.Module, run_dir: Path, suffix: str) -> Tuple[Path, Path]:
